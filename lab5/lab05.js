...
function showWindowHref(){
    ...
    var arr = url.value.substr(url.value.indexOf('?') + 1).split('&');
    arr.forEach(item=>{
        var tmp = item.split('=');
                    json[tmp[0]] = tmp[1];
    });
   document.getElementById("url-result").value = json['name'];
}
...
var timer = function timeTest(){
    let interval = setInterval(function(){
        document.getElementById("mul").value = document.getElementById("mul").value * 2;
        counter++;
        let date = new Date();
...
function arrSameStr(){
    ...
    document.getElementById("most-result").value = "The most character is:" + maxLetter + " times:" + levelNum;
